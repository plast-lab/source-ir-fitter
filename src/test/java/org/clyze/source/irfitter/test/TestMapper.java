...
public class TestMapper {
    ...
    void testJavaAndGroovy() throws IOException {
        ...
        assert (rr.unmatched == 52);
        assert (rr.idMapper.allTypes >= 38);
        assert (rr.idMapper.matchedTypes >= 38);
        assert (rr.idMapper.allMethods >= 194);
        assert (rr.idMapper.matchedMethods >= 194);
        assert (rr.idMapper.allFields >= 92);
        assert (rr.idMapper.matchedFields >= 92);
        assert (rr.idMapper.allInvos >= 1675);
        assert (rr.idMapper.matchedInvos >= 1606);
        assert (rr.idMapper.allAllocs >= 621);
        assert (rr.idMapper.matchedAllocs >= 620);
    ...
    void testKotlin() throws IOException {
        ...
        assert (rr.idMapper.allMethods >= 25);
        assert (rr.idMapper.matchedMethods >= 23);
        assert (rr.idMapper.allFields >= 46);
        assert (rr.idMapper.matchedFields >= 22);
        assert (rr.idMapper.allInvos >= 257);
        assert (rr.idMapper.matchedInvos >= 251);
        assert (rr.idMapper.allAllocs >= 23);
        assert (rr.idMapper.matchedAllocs >= 22);
        assert (rr.idMapper.allMethodRefs >= 0);
        assert (rr.idMapper.matchedMethodRefs >= 0);
        assert (rr.idMapper.allFieldAccesses >= 8);
        assert (rr.idMapper.matchedFieldAccesses >= 1);
